...
LoRa_DataRateTypeDef Value = LORA_CURRENT_DATA_RATE_SF10_BW125;
...
//	AT_AutoDateRate(AT_OPERATION_WRITE, &Value);
	AT_DataRateCommand(AT_OPERATION_READ, &Value);
	Value = LORA_CURRENT_DATA_RATE_SF7_BW125;
	AT_DataRateCommand(AT_OPERATION_WRITE, &Value);
	Value = LORA_CURRENT_DATA_RATE_SF12_BW125;
	AT_DataRateCommand(AT_OPERATION_READ, &Value);
	while (1) {
//		if (id != id)
